    ...
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-3">
        <StatCard
          ...
          value={String(jobs.filter((j) => j.status !== "closed").length)}
          accent="blue"
        />
        <StatCard
          title="Total applicants"
          value={String(jobs.reduce((s, j) => s + j.applicantsCount, 0))}
          delta="12 this week"
          deltaUp={true}
          accent="green"
        />
      ...
      {jobsQ.isLoading ? (
        ...
        <div className="space-y-2">
          {jobs.map((job) => (
            <div
              key={job.id}
              className="flex items-center justify-between rounded-xl border border-border p-4 transition-all hover:border-primary/20 hover:shadow-sm"
            >
              <div>
